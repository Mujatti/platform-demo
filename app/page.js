  ...
  useEffect(() => {
    function tryInit() {
      ...
      try {
        ...
        if (searchFieldEl) {
          searchFieldEl.addEventListener('submit', () => setTimeout(grabQueryAndFetch, 100), true);
        ...
        try {
          ...
          if (pending) {
            ...
            // Wait a tick for the search field to render, then fill and submit
            setTimeout(() => {
              const inp = document.querySelector(
                '#addsearch-searchfield input[type="text"], #addsearch-searchfield input[type="search"], #addsearch-searchfield input'
              );
              if (inp) {
                // Set value and dispatch events so AddSearch picks it up
